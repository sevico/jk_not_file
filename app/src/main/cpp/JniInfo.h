...
namespace JniInfo {
    extern std::string packageName;    // application package name
    extern std::string apkPath;        // path to apk file
    extern std::string libPath;        // path to native library
    extern std::string filePath;       // path to app file data
    extern std::string cachePath;      // path to .cache
    extern int sdk_int;

    extern JavaVM* vm;     // global java machine pointer
    extern bool isDalvik;
    extern bool isArt;
    ...
    bool attach(JavaVM *vm, JNIEnv *env = nullptr);
    // should be called in attach base context method to init global defination
    bool init(JNIEnv* env, jobject context_obj);
    // need to use DeleteLocalRef to delete return Ref obj.
    jobject getApplicaitonContext(JNIEnv* env = nullptr);
