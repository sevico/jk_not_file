 ...
 */
__inline int readUnsignedLeb128(const u1** pStream){
 ...
 * non-zero high-order bits in the fifth encoded byte.
 */
__inline DEX_INLINE int readSignedLeb128(const u1** pStream) {
    const u1 *ptr = *pStream;
    int result = *(ptr++);
 ...
 */
__inline int readAndVerifyUnsignedLeb128(const u1** pStream, const u1* limit,
        bool* okay) {
    const u1 *ptr = *pStream;
 ...
 * then the read is considered invalid.
 */
__inline int readAndVerifySignedLeb128(const u1** pStream, const u1* limit, bool* okay) {
    const u1 *ptr = *pStream;
 ...
 */
__inline u1* writeUnsignedLeb128(u1* ptr, u4 data) {
    while (true) {
        u1 out = data & 0x7f;
        if (out != data) {
            *ptr++ = out | 0x80;
 ...
 */
__inline int unsignedLeb128Size(u4 data){
    int count = 0;
