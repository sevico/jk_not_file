        ...
        ZipArchive* pArchive, off64_t fileLength, size_t readAmount, u1* scanBuf)
{
    ...
    ssize_t actual = (read(pArchive->mFd, scanBuf, sizeof(int32_t)));
    ...
    actual = (read(pArchive->mFd, scanBuf, readAmount));
    ...
    long* pModWhen, long* pCrc32)
{
    ...
    if (pOffset != NULL) {
        ...
        ssize_t actual =
            (read(pArchive->mFd, lfhBuf, sizeof(lfhBuf)));
...
static int inflateToFile(int outFd, int inFd, size_t uncompLen, size_t compLen)
{
    ...
    do {
        ...
        if (zstream.avail_in == 0) {
            ...
            ssize_t actual = (read(inFd, readBuf, getSize));
...
int dexZipExtractEntryToMem(const ZipArchive* pArchive,
    const ZipEntry entry, u1* &pMem, u4& len)
{
    int result = -1;
    int ent = entryToIndex(pArchive, entry);
    if (ent < 0) {
    ...
    auto align = page_size - uncompLen % page_size;
    auto pAlloc = mmap(nullptr, uncompLen + align,
    ...
    if (method == kCompressStored) {
        const size_t kBufSize = 32768;
        auto pWrite = (unsigned char*)pAlloc;
        auto count = uncompLen;
        while (count != 0) {
            size_t getSize = (count > kBufSize) ? kBufSize : count;

            ssize_t actual = (read(pArchive->mFd, pWrite, getSize));
            if (actual != (ssize_t) getSize) {
        ...
        const size_t kBufSize = 32768;
        unsigned char* readBuf =  new unsigned char[kBufSize];
        unsigned char* writeBuf = (unsigned char *) pAlloc;
        z_stream zstream;
        int zerr;
     ...
     */
        do {
            ...
            if (zstream.avail_in == 0) {
                ...
                ssize_t actual = (read(pArchive->mFd, readBuf, getSize));
                if (actual != (ssize_t) getSize) {
                    ALOGW("Zip: inflate read failed (%d vs %zd)",
                          (int)actual, getSize);
                    goto z_bail;
