...
void dvmHashIterNext(HashIter* pIter) {
    int i = pIter->idx +1;
...
void dvmHashIterBegin(HashTable* pHashTable, HashIter* pIter) {
    pIter->pHashTable = pHashTable;
    pIter->idx = -1;
    dvmHashIterNext(pIter);
}

bool dvmHashIterDone(HashIter* pIter) {
    return (pIter->idx >= pIter->pHashTable->tableSize);
}

void *dvmHashIterData(HashIter* pIter) {
